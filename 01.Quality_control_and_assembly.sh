...
# 3.指定原始rawData数据存放目录
OD=/data/xb/metagenome/yakseq

# 指定宿主基因组存放路径
HG=/data/xb/metagenome/genome

# 指定宿主基因组名称, 普氏原羚用的是自己组装的基因组.
GF="Yak.fa"

# 4. 指定结果存放的根目录
WP=/data/xb/metagenome
...
do
  ...
  then
    ...
    filename=$(basename $FILE .raw.fastq.gz | cut -d. -f1)
    echo $filename
  fi
done | uniq)
wait
...
for FID in $filenames
do
	samtools view -@ 8 -bS ${BSAM}/${FID}.bwa.sam | samtools sort -@ 8  -o ${BBAM}/${FID}.sorted.bam &
	count=$((count+1))
	if [ $count -eq $parallel ]; then
	wait
	count=0
...
for FID in $filenames
do
	samtools merge -@ 8 ${HF}/${FID}.hostFree.bam ${HF}/${FID}.bothUnmapped.bam ${HF}/${FID}.R1Unmapped.bam ${HF}/${FID}.R2Unmapped.bam &
	count=$((count+1))
	if [ $count -eq $parallel ]; then
	wait
	count=0
...
for FID in $filenames
do
	samtools sort -@ 8  -o ${HF}/${FID}.hostFree.sorted.bam ${HF}/${FID}.hostFree.bam &
	count=$((count+1))
	if [ $count -eq $parallel ]; then
	wait
	count=0
